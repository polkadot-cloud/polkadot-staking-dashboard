...
export type UseAuthChallengeMutation = {
	authChallenge: (options?: { variables: { address: string } }) => Promise<{
		data?: { authChallenge: AuthChallengeResult }
	}>
	loading: boolean
	error: ApolloError | undefined
	data?: { authChallenge: AuthChallengeResult }
...
export type UseAuthResponseMutation = {
	authResponse: (options?: {
		variables: { address: string; challengeId: string; signature: string }
	}) => Promise<{
		data?: { authResponse: AuthResponseResult }
	}>
	loading: boolean
	error: ApolloError | undefined
	data?: { authResponse: AuthResponseResult }
