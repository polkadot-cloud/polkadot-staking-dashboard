...
export const ConnectProvider = ({
  ...
  const forgetAccounts = useCallback(
    (_accounts: Array<ExternalAccount>) => {
    ...
    [activeAccount, network, setActiveAccount]
  );
  ...
  const handleExtensionError = useCallback((id: string, err: string) => {
              ...
              const _unsubscribe = (await extension.accounts.subscribe(
                (injected: ExtensionAccount[]) => {
                  ...
                  if (injected.length) {
                    ...
                    // reformat address to ensure correct format
                    injected.forEach(async (account: ExtensionAccount) => {
                      const { address } = keyring.addFromAddress(
                        account.address
                      );
                      account.address = address;
                      return account;
  ...
  useEffect(() => {
    ...
    const importExternalAccounts = () => {
      ...
      if (localExternalAccounts.length) {
        ...
        const _accounts = [...accountsRef.current].concat(
  ...
  }, [connectToAccount, extensionsFetched, network]);
